...
- 🔴 **تسليم v1.5.0 (تحت) غلط — كان مجرد إعادة تسمية تاب "Delivered" لـ"الأرشيف"،
  من غير أي منطق تجميع فعلي.** أحمد رفض التسليم وصحّح المطلوب: "الأرشيف" لازم
  يكون تاب **جديد ومنفصل**، و"Delivered" يرجع باسمه ومكانه الأصليين.
- **تاب "✅ Delivered" رجع زي ما كان قبل v1.5.0** — بس كود حالة 45، بغض النظر
  عن `write_status`، مش متأثر بأي منطق أرشفة.
- **تاب "🗄 الأرشيف" بقى تاب جديد فعليًا** (آخر تاب في الترتيب): بمجرد ما
  `tracking_number` معيّن يوصله حدث بكود حالة 45 (Delivered) ولو مرة واحدة،
  **كل سجلاته** (بكل الحالات اللي مرّ بيها — 21/24/30/41/45/...، مش كود 45
  بس) بتتجمّع في تاب الأرشيف، **وبتتشال بالكامل من تاب "🚚 الأوردرات تحت
  التوصيل"**. حسابها بـ `computeArchivedTrackingNumbers(rawEvents)` — Set بكل
  `tracking_number` عنده حدث `state === 45`، بيتحسب مرة واحدة بعد كل
  `loadEvents()` وبيتقرا في `match` بتاعة تابي `main`/`archive` بس.
- 🔴 **الأرشفة على مستوى `tracking_number` مش `business_reference`.** لو
  الأوردر عنده أكتر من رقم تتبع (S1 وS2 — شحن أصلي + استرجاع/استبدال مثلًا)
  ورقم واحد بس وصل Delivered، سجلات الرقم التاني (لسه شغّال) بتفضل في
  "الأوردرات تحت التوصيل" عادي — مش الأوردر كله بيتشال دفعة واحدة.
- 🔴 **تابات "✅ Delivered"/"⚠️ Exception"/"↩️ Returned to business" مش
  بيستبعدوا أرقام التتبع المؤرشفة.** لو رقم تتبع وصل Delivered وكان عنده حدث
  Exception قبل كده، الحدث ده بيفضل ظاهر في تاب Exception **وكمان** في تاب
  الأرشيف (نفس الصف في تابين) — الأرشيف عرض تجميعي إضافي، مش بديل عن التابات
  التانية. الاستبعاد الوحيد المطلوب صراحةً كان من تاب "الأوردرات تحت
  التوصيل".
- **مفيش تغيير Worker** — الحساب كله client-side على `rawEvents` المحمّلة
  فعلاً. `TOOL_VERSION` اتصعّد لـ `1.6.0`، `WORKER_VERSION`/`MIN_WORKER_VERSION`
  فضلوا `1.5.0`.
- اتّحقق بـ Playwright headless (Worker مُحاكى): أوردر بـ 5 أحداث غير-45 +
  حدثين Delivered → السبعة اتلمّوا في الأرشيف، الخمسة اتشالوا من "الأوردرات
  تحت التوصيل" (وفضل بس أوردر تاني مش مؤرشف)، وتاب Delivered عرض الحدثين
  الـ45 مستقلين زي ما هو متوقّع.
